 ...
 * 更新する必要があります.
 */
export function RawDeflate(input, opt_params) {
  /** @type {Zlib.RawDeflate.CompressionType} */
  this.compressionType = this.CompressionType.DYNAMIC;
  /** @type {number} */
  this.lazy = 0;
 ...
 * @enum {number}
 */
RawDeflate.prototype.CompressionType = {
  NONE: 0,
  FIXED: 1,
  DYNAMIC: 2,
  RESERVED: 3
 ...
 * @type {number}
 */
RawDeflate.prototype.Lz77MinLength = 3;
 ...
 * @type {number}
 */
RawDeflate.prototype.Lz77MaxLength = 258;
 ...
 * @type {number}
 */
RawDeflate.prototype.WindowSize = 0x8000;
 ...
 * @type {number}
 */
RawDeflate.prototype.MaxCodeLength = 16;
 ...
 * @type {number}
 */
RawDeflate.prototype.HUFMAX = 286;
 ...
 */
RawDeflate.prototype.FixedHuffmanTable = (function() {
  var table = [], i;
...
RawDeflate.prototype.compress = function() {
  ...
  switch (this.compressionType) {
    case this.CompressionType.NONE:
      ...
      break;
    case this.CompressionType.FIXED:
      this.output = this.makeFixedHuffmanBlock(input, true);
      this.op = this.output.length;
      break;
    case this.CompressionType.DYNAMIC:
...
function(blockArray, isFinalBlock) {
  ...
  btype = this.CompressionType.NONE;
...
function(blockArray, isFinalBlock) {
  ...
  // header
  bfinal = isFinalBlock ? 1 : 0;
  btype = this.CompressionType.FIXED;
...
function(blockArray, isFinalBlock) {
  ...
  // header
  bfinal = isFinalBlock ? 1 : 0;
  btype = this.CompressionType.DYNAMIC;
...
RawDeflate.prototype.fixedHuffman = function(dataArray, stream) {
  ...
  for (index = 0, length = dataArray.length; index < length; index++) {
    ...
    Zlib.BitStream.prototype.writeBits.apply(
      stream,
      this.FixedHuffmanTable[literal]
    );
 ...
 * @constructor
 */
RawDeflate.prototype.Lz77Match = function(length, backwardDistance) {
 ...
 * @type {!(Array.<number>|Uint32Array)}
 */
RawDeflate.prototype.Lz77Match.LengthCodeTable = (function(table) {
  ...
  function code(length) {
    switch (true) {
      ...
      case (length === 8): return [262, length - 8, 0];
 ...
 * @private
 */
RawDeflate.prototype.getDistanceCode_ = function(dist) {
 ...
 */
RawDeflate.prototype.toLz77Array = function() {
  /** @type {number} */
  ...
  // length
  code = this.Lz77Match.LengthCodeTable[length];
...
RawDeflate.prototype.lz77 = function(dataArray) {
  ...
  for (position = 0, length = dataArray.length; position < length; ++position) {
    // ハッシュキーの作成
    for (matchKey = 0, i = 0, il = this.Lz77MinLength; i < il; ++i) {
    ...
    if (position + this.Lz77MinLength >= length) {
      if (prevMatch) {
...
function(data, position, matchList) {
  ...
  for (i = 0, l = matchList.length; i < l; i++) {
    ...
    matchLength = this.Lz77MinLength;

    // 前回までの最長一致を末尾から一致検索する
    if (matchMax > this.Lz77MinLength) {
      for (j = matchMax; j > this.Lz77MinLength; j--) {
    ...
    // 最長一致探索
    while (matchLength < this.Lz77MaxLength &&
    ...
    // 最長が確定したら後の処理は省略
    if (matchLength === this.Lz77MaxLength) {
      break;
    }
  }

  return new this.Lz77Match(matchMax, position - currentMatch);
};
...
RawDeflate.prototype.getLengths_ = function(freqs, limit) {
  ...
  /** @type {Zlib.Heap} */
  var heap = new Zlib.Heap(2 * this.HUFMAX);
  /** @type {!(Array.<number>|Uint8Array)} */
...
RawDeflate.prototype.getCodesFromLengths_ = function(lengths) {
  ...
  for (i = 1, il = this.MaxCodeLength; i <= il; i++) {
    startCode[i] = code;
    code += count[i] | 0;
    code <<= 1;
  }
